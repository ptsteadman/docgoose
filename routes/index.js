...
exports.index = function (req, res) {
	...
	if (location) {
		sortSchoolsByDistanceFrom(schools,location.ll[0],location.ll[1]);
		console.log(location);	
		res.redirect('/' + schools[0].name);
	} else {
		res.redirect('/cornell'); //Cornell
	}	
};

function sortSchoolsByDistanceFrom(schools, latitude, longitude) {
	console.log("sorting schools");
	
	var length = schools.length;
	for (var i = 0; i < length; i++) {
		console.log(schools[i].name + " : ");computeDistanceFromSchool(schools[i], latitude, longitude);
	}
...
function computeDistanceFromSchool(school, latitude, longitude) {
	school.distance = computeDistanceBetween(school.latitude, school.longitude, latitude, longitude);
}

function computeDistanceBetween(latitude1, longitude1, latitude2, longitude2) {
